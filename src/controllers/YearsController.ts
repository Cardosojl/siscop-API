...
class YearsController {
    ...
    async index(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (years?.length === 0) return res.status(400).json({ error: 'Year não encontrado!' });
    ...
    async show(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (!year) {
                return res.status(404).json({ error: 'Year não encontrado!' });
            }
    ...
    async update(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (!year) return res.status(404).json({ error: 'Year não encontrado!' });
